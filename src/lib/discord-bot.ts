...
function responseText(title: string, description: string) {
  return `**${title}**\n${description}`;
}
...
function extractAdamRequest(message: string) {
  const match = message.trim().match(
    /^(?:hey\s+adam|adam)\b(?:\s*[,!?:;-]\s*|\s+)(.+)$/i,
  );
  return match?.[1]?.trim() ?? "";
}
...
async function handleVerificationHelp(message: Message) {
  ...
  await message.reply({
    content: verificationInfoText(message.guild, message.member),
...
async function handleVerifyCommand(message: Message) {
  ...
  if (!result.ok) {
    await message.reply({
      content: responseText(result.title, result.description),
      allowedMentions: { repliedUser: false },
    });
    return;
  }
...
async function handleRequest(
  ...
  if (!request) {
    await reply({
      content:
        "Ask me a question or describe what you need help with. Try asking about the weather, this server, team priorities, drafts, or recent activity.",
    });
    return null;
  }
    ...
    await interaction.reply({
      embeds: [
        responseEmbed(
          ...
          "That LOA date form expired. Please start a new request such as `LOA: vacation request`.",
          0xed4245,
        ),
      ],
      ephemeral: true,
...
export function startDiscordBot() {
  ...
  client.on(Events.MessageCreate, async (message: Message) => {
    ...
    try {
      ...
      const requestText = extractAdamRequest(message.content);
      if (!requestText) {
        return;
      }
      ...
      await message.reply({
        content: "I could not respond to that message. Nothing was changed.",
        allowedMentions: { repliedUser: false },
      });
    }
  });
